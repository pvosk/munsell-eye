(() => {
  ...
  // clockwork/js/workers/osc_out_log_sab_worker.js
  var reportedMissed = 0;
  function readLogMessages(ctx) {
    ...
    if (missed > reportedMissed) {
      if (true) {
        console.warn(`[OSCOutLogWorker] ${missed - reportedMissed} messages were overwritten before they could be logged (${missed} since start)`);
      }
