...
function midi_out_decode(osc2) {
  const ptr0 = passArray8ToWasm0(osc2, wasm.__wbindgen_malloc);
  const len0 = WASM_VECTOR_LEN;
...
var MidiManager = class {
  ...
  _onInput(port, event) {
    ...
    const osc2 = midi_in_osc_at(port, bytes, when);
    if (osc2 && this._onEvent) this._onEvent(osc2);
  }
  // ── Sends ────────────────────────────────────────────────────────────────
...
function gamepad_out_decode(osc2) {
  const ptr0 = passArray8ToWasm02(osc2, wasm2.__wbindgen_malloc);
  const len0 = WASM_VECTOR_LEN2;
...
var GamepadManager = class {
  ...
  _emit(pad, kind, name, a, b) {
    ...
    const osc2 = kind === "button" ? gamepad_button_osc_at(pad, name, a, b, when) : gamepad_axis_osc_at(pad, name, a, when);
    this._onEvent(osc2);
  }
...
var HostFront = class {
  ...
  async init() {
    if (this._midiOptions) {
      ...
      this._midi.onEvent((osc2) => this._ingest(osc2));
      this._midi.onPorts((osc2) => this._ingest(osc2));
    ...
    if (this._gamepadOptions) {
      ...
      this._gamepad.onEvent((osc2) => this._ingest(osc2));
      this._gamepad.onDevices((osc2) => this._ingest(osc2));
