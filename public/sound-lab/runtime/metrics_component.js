// clockwork/js/lib/metrics_component.js
var HEADROOM_UNSET = 4294967295;
...
function makeFormatter(cell, metricDef, metrics) {
  ...
  if (cell.format === "chromeOnly") {
    return (v, arr) => hpsOffset !== void 0 && arr && !arr[hpsOffset] ? "-" : String(v);
  }
  if (cell.format === "chromeLatencyUs") {
    return (v, arr) => hpsOffset !== void 0 && arr && !arr[hpsOffset] ? "-" : formatLatencyUs(v);
  }
  return (v) => String(v);
}
var ClockworkMetrics = class extends HTMLElement {
  #sonic = null;
  #intervalId = null;
  #cells = [];
  // { offset, format, el, prev } — flat binding array
  #bars = [];
  // { usedOffset, peakOffset, capacityOffset, fillEl, peakEl, labelEl, prevUsed, prevPeak, prevCap }
  #built = false;
  /**
  ...
  connect(sonic, options = {}) {
    ...
    this.#intervalId = setInterval(() => this.#update(), 1e3 / hz);
  }
  /**
