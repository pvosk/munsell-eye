...
const inputs=['docs/play-hard-lab-selection.json'];
const chosen=JSON.parse(readFileSync(inputs[0],'utf8')).results.filter((r:any)=>r.accepted);
const rows:any[]=[];
for(const x of chosen){
 let row=rows.find(r=>r.trial.name===x.job.name);
 if(!row){row={trial:{name:x.job.name,paints:x.job.paints},core:x.job.core,value:x.job.value,file:inputs[0],deep:[]};rows.push(row);}
 row.deep.push({checked:x.checked,proposal:{id:x.job.id,target:x.job.target,recipe:x.job.recipe??[]},core:x.job.core});
}
const names=rows.map(r=>r.trial.name);
const path='app/generated/play-lab-round11.json';
...
for(const [i,row] of rows.entries()){
 ...
 for(const [stage,h] of survivors.entries()){
  ...
  const label={interior:c.bases.every((b:any)=>b.rawFewest===3)?'Three-addition setup':'Multi-start setup',ride:'Chromatic ride opportunity','value-shift':'Value-shift finish',balance:'Hue/value balancing',glide:'Setup + long finish',direct:'Direct mixing contrast'}[style]!;
