...
const output='docs/play-expanded-portfolios.json';if(existsSync(output))throw Error('Preserve existing portfolio run');
const sources=['scripts/search-bank-portfolios.ts','scripts/broad-palette-mine.ts','scripts/backward-verification.ts','scripts/recipe-blind-search.ts','scripts/stable-route-measurement.ts','app/play-inverse-planning.ts','app/play-engine.ts','app/paint-mixing.ts','app/play-route-design.ts','app/play-route-analysis.ts','app/play-journey-analysis.ts','app/play-experience-audit.ts','app/play-finish-profile.ts'];
const sourceHash=createHash('sha256').update(sources.map(p=>p+readFileSync(p,'utf8')).join('\n')).digest('hex');
const policy={version:'expanded-portfolios-1',samples:60,screenTargets:12,deepTargets:3,minimumTargetSeparation:T*2,seed:91231300};
const sets=[
 {indices:[6,15,20],role:'muted-triad: ochre/maroon/cerulean, no white or black'},
 {indices:[7,18,28],role:'muted-triad: light chromatic pigment with blue and sienna'},
 {indices:[8,16,31],role:'restricted-triad: pale yellow, dark violet and muted green'},
 {indices:[5,13,21],role:'chromatic-triad: yellow/magenta/blue without white'},
 {indices:[1,5,13,24],role:'ride-reference: yellow/magenta/green with white'},
 {indices:[9,14,19,30],role:'dark-correction: orange/crimson/cobalt/umber'},
 {indices:[1,6,11,18,25],role:'wide-five: earth and chromatic paints with white'},
 {indices:[8,12,16,20,28],role:'wide-five: chromatic value changes without white'}
];
