...
const inputs=['docs/play-green-portfolios.json','docs/play-expanded-portfolios.json'];
const rows=inputs.flatMap(file=>JSON.parse(readFileSync(file,'utf8')).results.map((r:any)=>({...r,file})));
...
const path='app/generated/play-lab-round10.json';
...
for(const [i,row] of rows.entries()){
 ...
 for(const [stage,h] of survivors.entries()){
  ...
  const style=allSetup?'interior':all.some((r:any)=>has(r,preferred))?preferred:all.some((r:any)=>r.glide)?'glide':all.some((r:any)=>has(r,'balance'))?'balance':'direct';
  ...
  const label={interior:'Multi-start setup',ride:'Chromatic ride opportunity','value-shift':'Value-shift finish',balance:'Hue/value balancing',glide:'Setup + long finish',direct:'Direct mixing contrast'}[style]!;
