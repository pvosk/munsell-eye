...
import publicCampaign from './generated/play-public-campaign.json';
...
export const publicCampaignBank=publicCampaign as unknown as {seed:number;tolerance:number;holes:PremixRow[];chapters:{name:string;level:number;ids:string[]}[]};
export const allPremixRows:PremixRow[]=[...bank.holes,...regionBank.holes,...legLabBank.holes,...branchLabBank.holes,...conditionedLabBank.holes,...workbenchBank.holes,...publicCampaignBank.holes];
export function generatePremixHole(pairId:string,mode:MassMode):Hole{
 ...
 const r=item.modes[mode]!,target=colorPoint([...item.targetRGB] as [number,number,number]),source=item.collection==='public'?publicCampaignBank:item.collection==='workbench'?workbenchBank:item.collection==='conditioned'?conditionedLabBank:item.collection==='branches'?branchLabBank:item.collection==='legs'?legLabBank:item.role?regionBank:bank;
 return{seed:source.seed,stage:item.role?0:item.stage,start:item.freeBase?neutralStart(PLAY_LEVELS[item.level]):mixtureColor(paints,item.initial),target,notation:nearestNotation(target),par:item.par??3,recipe:premixReplay(item.initial,r.order,r.times,mode,item.freeBase),tolerance:source.tolerance,timingWindow:r.measurement.finishWindowMs/1000,courseId:`${item.id}-${mode}`,kind:item.style,solutionShots:r.minimum,routeOrder:[...r.order],routeTimes:[...r.times],premix:{initial:[...item.initial],massMode:mode,pairId,...(item.freeBase?{freeBase:true as const}:{})}};
}
export function premixCollection(hole:Hole){const item=premixItem(hole);return item?.collection==='public'?publicCampaignBank:item?.collection==='workbench'?workbenchBank:item?.collection==='conditioned'?conditionedLabBank:item?.collection==='branches'?branchLabBank:item?.collection==='legs'?legLabBank:item?.role?regionBank:premixBank;}
...
export const PUBLIC_SPECIMENS=publicCampaignBank.holes.map(h=>({levelIndex:h.level,hole:generatePremixHole(h.id,'normalized')}));
