...
export const allPremixRows:PremixRow[]=[...bank.holes,...regionBank.holes,...legLabBank.holes];
export function generatePremixHole(pairId:string,mode:MassMode):Hole{
 ...
 const r=item.modes[mode]!,target=colorPoint([...item.targetRGB] as [number,number,number]),source=item.collection==='legs'?legLabBank:item.role?regionBank:bank;
...
export function premixCollection(hole:Hole){const item=premixItem(hole);return item?.collection==='legs'?legLabBank:item?.role?regionBank:premixBank;}
