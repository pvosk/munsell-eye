...
import {PremixPicker,PremixRouteReview,CourseWorkbenchPicker} from './play-premix-ui';
import {premixItem,PUBLIC_SPECIMENS} from './play-premix-bank';
...
export function LabPicker(props:{current:LabSpecimen;onChoose:(s:LabSpecimen)=>void}) {
 ...
 const [view,setView]=useState(()=>current.hole.premix?(premixItem(current.hole)?.collection??(premixItem(current.hole)?.role?'regions':'premix')):campaignForHole(current.hole.courseId)?'campaign':current.hole.courseId.startsWith('lab-12-')?'fresh':current.hole.courseId.startsWith('lab-11-')?'hard':current.hole.courseId.startsWith('lab-10-')?'inverse':'workbench'),[selected,setSelected]=useState(()=>Math.max(0,CAMPAIGN_CHAPTERS.findIndex(c=>c.levelIndex===current.levelIndex)));
 ...
 return <div className="lab-campaign-picker"><label>Test collection<select value={view} onChange={e=>setView(e.target.value)}><option value="public">New · Premix campaign · 17 chapters</option><option value="workbench">Previous · Course workbench</option><option value="conditioned">Previous · 16 destination branching puzzles</option><option value="branches">Previous · 4 branching setups</option><option value="legs">Previous · 10 contrasting pigment-leg puzzles</option><option value="regions">Previous · 11 normalized premix puzzles</option><option value="premix">Archive · 4 puzzles × A/B mass</option><option value="fresh">6 harder holes · newer palettes</option><option value="hard">Previous · 8 harder setup holes</option><option value="inverse">30 inverse-planning holes</option><option value="campaign">Archive · Free-base campaign</option><option value="archive">All lab rounds</option></select></label>
 {view==='public'?<InverseLabPicker key="public" {...props} bank={PUBLIC_SPECIMENS}/>:view==='workbench'?<CourseWorkbenchPicker {...props}/>:view==='conditioned'?<PremixPicker key="conditioned" collection="conditioned" {...props}/>:view==='branches'?<PremixPicker key="branches" collection="branches" {...props}/>:view==='legs'?<PremixPicker key="legs" collection="legs" {...props}/>:view==='regions'?<PremixPicker key="regions" {...props}/>:view==='premix'?<PremixPicker key="premix" archive {...props}/>:view==='fresh'?<InverseLabPicker key="fresh" {...props} bank={LAB_FRESH}/>:view==='hard'?<InverseLabPicker key="hard" {...props} bank={LAB_HARD}/>:view==='inverse'?<InverseLabPicker key="inverse" {...props} bank={LAB_INVERSE}/>:view==='archive'?<LegacyLabPicker {...props}/>:<>
...
function InverseLabPicker({current,onChoose,bank}:{current:LabSpecimen;onChoose:(s:LabSpecimen)=>void;bank:LabSpecimen[]}) {
 ...
 return <>
  ...
  <label>Hole in this palette<select value={at<0?'':items[at].hole.courseId} onChange={e=>choose(items.find(s=>s.hole.courseId===e.target.value)!)}>{at<0&&<option value="">Choose a new hole…</option>}{items.map((s,i)=><option key={s.hole.courseId} value={s.hole.courseId}>{i+1}. {premixItem(s.hole)?.title??directedForHole(s.hole.courseId)?.label} · {s.hole.notation}</option>)}</select></label>
  ...
  <details><summary>Paints & hole intent{row?` · ${row.label}`:''}</summary><p>{PLAY_LEVELS[level].paints.map(p=>p.name).join(' · ')}</p><p>{bank===PUBLIC_SPECIMENS?'Fixed prepared mixtures, normalized after every pour. Explore any chapter and replay freely. Par is a provisional player allowance, not a pigment-leg minimum.':row?.brief??'Choose a test palette and hole. Skip freely and replay as often as you like. Earlier reviews remain available.'}</p><p>Pigment strengths and landing tolerance are preserved. Live touch response has been refined; archived examples retain their original dose recipes.</p></details>
 </>;
}
