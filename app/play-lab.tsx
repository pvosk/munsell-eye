...
export function LabPicker(props:{current:LabSpecimen;onChoose:(s:LabSpecimen)=>void}) {
 ...
 const [view,setView]=useState(()=>current.hole.premix?(premixItem(current.hole)?.collection==='legs'?'legs':premixItem(current.hole)?.role?'regions':'premix'):campaignForHole(current.hole.courseId)?'campaign':current.hole.courseId.startsWith('lab-12-')?'fresh':current.hole.courseId.startsWith('lab-11-')?'hard':current.hole.courseId.startsWith('lab-10-')?'inverse':'legs'),[selected,setSelected]=useState(()=>Math.max(0,CAMPAIGN_CHAPTERS.findIndex(c=>c.levelIndex===current.levelIndex)));
