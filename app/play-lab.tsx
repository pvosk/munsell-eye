...
export function PlayLabPanel({sync,current,onReplay,onReveal}:{sync:ReturnType<typeof usePlayLabSync>;current:LabAttempt|null;onReplay:(specimen:LabSpecimen)=>void;onReveal:()=>void}) {
  ...
  return <section className="play-lab-panel" aria-label="Hole design lab">
    ...
    {!sync.signedIn?<p><a href="/signin-with-chatgpt?return_to=%2F%3Fmode%3Dplay%26lab%3D1" target="_top">Sign in with ChatGPT</a> to save private attempts and notes across devices.</p>:<>
      ...
      <p className="lab-muted">Round 8: six robust interior/reference holes, one no-white balance, one deliberate close-start detour, and two experimental chromatic rides. These use frozen measured routes from the deeper palette search. Rides retain alternative approaches; they are not guaranteed from every start. Par rules and landing calibration are unchanged. Earlier rounds and your reviews remain available.</p>
      ...
      {entry&&<div className="lab-review"><h3>Feedback · {PLAY_LEVELS[entry.attempt.specimen.levelIndex].name}</h3><ReviewForm key={entry.attempt.id} entry={entry} onSave={review=>sync.save({id:crypto.randomUUID(),attemptId:entry.attempt.id,type:'review',review})}/></div>}
    </>}
  </section>;
}
