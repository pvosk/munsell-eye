...
export function LabPicker({current,onChoose,disabled}:{current:LabSpecimen;onChoose:(s:LabSpecimen)=>void;disabled:boolean}) {
  ...
  </select></label>{index<tests.length-1&&<button type="button" disabled={disabled} onClick={()=>choose(tests[index+1])}>{index<0?'Start round 6':'Next test →'}</button>}{directed?<p className="lab-muted"><strong>{directed.label} · {directed.emphasis==='experience'?'Experience-led candidate':'Starting-flexibility candidate'}.</strong> Choose your base freely. Review reveals the intended route and competing shortcuts.</p>:focused?<p className="lab-muted"><strong>{focused.coverage.styleBases.length}/{focused.coverage.total} starts support this style.</strong> {focused.brief} All starts meet route-support checks; choose your base freely.</p>:experiment&&<p className="lab-muted">{experiment.reference?'Reference · ':''}{experiment.brief} Free base choice; the brief is not an instruction to take a particular route.</p>}</>;
}
...
export function PlayLabPanel({sync,current,onReplay,onReveal}:{sync:ReturnType<typeof usePlayLabSync>;current:LabAttempt|null;onReplay:(specimen:LabSpecimen)=>void;onReveal:()=>void}) {
  ...
  return <section className="play-lab-panel" aria-label="Hole design lab">
    ...
    {!sync.signedIn?<p><a href="/signin-with-chatgpt?return_to=%2F%3Fmode%3Dplay%26lab%3D1" target="_top">Sign in with ChatGPT</a> to save private attempts and notes across devices.</p>:<>
      ...
      <p className="lab-muted">Round 6: eight directed tests across four styles. Each pair compares an experience-led candidate with a starting-flexibility candidate. Play naturally first, then review the intended route, the fewest-addition route found, and a closest-paint start. Tell us whether the style came through and whether shortcuts were enjoyable. These are the strongest candidates in this search, not globally optimal holes. Par rules and landing calibration are unchanged. Earlier rounds remain available; retired palettes can only be replayed from your history.</p>
