...
import {supportedLabEngine,LAB_GROUPS,LAB_DESIGN,directedForHole,analysisForHole,suggestedComparison,labEntries,type LabAttempt,type LabReview,type LabSpecimen,type LabEntry} from './play-lab-model';
...
const RouteReview=memo(function RouteReview({entry}:{entry:LabEntry}) {
  ...
  const all=[...points,...referenceBounds,target];
...
export function LabPicker({current,onChoose,disabled}:{current:LabSpecimen;onChoose:(s:LabSpecimen)=>void;disabled:boolean}) {
  ...
  </select></label>{index<tests.length-1&&<button type="button" disabled={disabled} onClick={()=>choose(tests[index+1])}>{index<0?'Start round 7':'Next test →'}</button>}{directed?<p className="lab-muted"><strong>{directed.label} · {directed.emphasis==='experience'?'Experience-led candidate':'Starting-flexibility candidate'}.</strong> {current.hole.courseId.startsWith('lab-7-')&&directed.brief} Choose your base freely. Review reveals the intended route and competing shortcuts.</p>:focused?<p className="lab-muted"><strong>{focused.coverage.styleBases.length}/{focused.coverage.total} starts support this style.</strong> {focused.brief} All starts meet route-support checks; choose your base freely.</p>:experiment&&<p className="lab-muted">{experiment.reference?'Reference · ':''}{experiment.brief} Free base choice; the brief is not an instruction to take a particular route.</p>}</>;
}
...
export function PlayLabPanel({sync,current,onReplay,onReveal}:{sync:ReturnType<typeof usePlayLabSync>;current:LabAttempt|null;onReplay:(specimen:LabSpecimen)=>void;onReveal:()=>void}) {
  ...
  return <section className="play-lab-panel" aria-label="Hole design lab">
    ...
    {!sync.signedIn?<p><a href="/signin-with-chatgpt?return_to=%2F%3Fmode%3Dplay%26lab%3D1" target="_top">Sign in with ChatGPT</a> to save private attempts and notes across devices.</p>:<>
      ...
      <p className="lab-muted">Round 7: three protected interior setups, three value-shift tests (up, down, or either), and two contrasting chromatic rides. Intended styles stay visible. Play from your preferred base, then replay from another and compare routes. Interiors passed the three-addition checks from every base; shifts and rides are opportunities with known alternatives, not guaranteed experiences. These tests come from the frozen searches, not the newer experimental palette optimizer. Par rules and landing calibration are unchanged. Earlier rounds and your reviews remain available.</p>
      ...
      {reviewing&&entry&&<div className="lab-review" key={entry.attempt.id}>
        ...
        {directed&&<p className="lab-muted">Style opportunity from {directed.coverage.availableBases.length}/{directed.analysis.bases.length} starting paints. It survives every retained efficient alternative from {directed.coverage.robustBases.length}/{directed.analysis.bases.length} starts. {directed.coverage.eligible?'Passes the stricter style-resistance checks.':'Does not pass every stricter style-resistance check; included to test whether the intended experience remains worthwhile.'} Search is sampled; alternatives may overlap in style.</p>}
