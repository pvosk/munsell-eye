...
export function LabPicker(props:{current:LabSpecimen;onChoose:(s:LabSpecimen)=>void}) {
 ...
 const [view,setView]=useState(()=>current.hole.premix?'premix':campaignForHole(current.hole.courseId)?'campaign':current.hole.courseId.startsWith('lab-10-')?'inverse':current.hole.courseId.startsWith('lab-11-')?'hard':'fresh'),[selected,setSelected]=useState(()=>Math.max(0,CAMPAIGN_CHAPTERS.findIndex(c=>c.levelIndex===current.levelIndex)));
 ...
 return <div className="lab-campaign-picker"><label>Test collection<select value={view} onChange={e=>setView(e.target.value)}><option value="premix">Premix · 4 puzzles × A/B mass</option><option value="fresh">6 harder holes · newer palettes</option><option value="hard">Previous · 8 harder setup holes</option><option value="inverse">30 inverse-planning holes</option><option value="campaign">Campaign · one palette at a time</option><option value="archive">All lab rounds</option></select></label>
 {view==='premix'?<PremixPicker {...props}/>:view==='fresh'?<InverseLabPicker key="fresh" {...props} bank={LAB_FRESH}/>:view==='hard'?<InverseLabPicker key="hard" {...props} bank={LAB_HARD}/>:view==='inverse'?<InverseLabPicker key="inverse" {...props} bank={LAB_INVERSE}/>:view==='archive'?<LegacyLabPicker {...props}/>:<>
