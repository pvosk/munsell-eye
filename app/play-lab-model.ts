...
export const supportedLabEngine=(engine:string)=>[LAB_ENGINE,'glider-courses-2-controls-1',LAB_ROUND_ENGINE,LAB_PAIRED_ENGINE,LAB_DESIGN_ENGINE,LAB_FOCUSED_ENGINE,LAB_DIRECTED_ENGINE].includes(originalEngine(engine));
...
export const directedForHole=(id:string)=>directedLabBank.holes.find(h=>h.record.id===id);
export const labHoleProgress=(hole:Hole)=>hole.courseId.startsWith('lab-6-')?`${LAB_DIRECTED.findIndex(s=>s.hole.courseId===hole.courseId)+1}/8`:`${hole.stage+1}/${hole.courseId.startsWith('lab-5-')?2:5}`;
...
export function nextFixedLabSpecimen(current:LabSpecimen):LabSpecimen|null{
  const bank=current.hole.courseId.startsWith('lab-6-')?LAB_DIRECTED:current.hole.courseId.startsWith('lab-5-')?LAB_FOCUSED:current.hole.courseId.startsWith('lab-4-')?LAB_DESIGN:current.hole.courseId.startsWith('lab-3-')?LAB_PAIRED:current.hole.courseId.startsWith('lab-2-')?LAB_STARTERS:null;
  if(!bank)return null;
...
export function newLabAttempt(specimen:LabSpecimen,id:string):LabAttempt {
  const base=specimen.hole.courseId.startsWith('lab-6-')?LAB_DIRECTED_ENGINE:specimen.hole.courseId.startsWith('lab-5-')?LAB_FOCUSED_ENGINE:specimen.hole.courseId.startsWith('lab-4-')?LAB_DESIGN_ENGINE:specimen.hole.courseId.startsWith('lab-3-')?LAB_PAIRED_ENGINE:specimen.hole.courseId.startsWith('lab-2-')?LAB_ROUND_ENGINE:specimen.hole.courseId.startsWith('courses-2-')?'glider-courses-2-controls-1':LAB_ENGINE;
...
export function validLabEvent(value:unknown):value is LabEvent {
  ...
  if(s.comparison&&(![LAB_PAIRED_ENGINE,LAB_DESIGN_ENGINE,LAB_FOCUSED_ENGINE,LAB_DIRECTED_ENGINE].includes(engine)||!id(s.comparison.sourceAttemptId)||!Number.isInteger(s.comparison.base)||!PLAY_LEVELS[s.levelIndex].paints[s.comparison.base]))return false;
