import {PREMIX_ENGINE,PREMIX_SPECIMENS,REGION_SPECIMENS,LEG_SPECIMENS,generatePremixHole,allPremixRows,premixItem,premixProgress} from './play-premix-bank';
...
export const LAB_GROUPS=[
  {name:'New · Ten contrasting pigment-leg puzzles',items:LEG_SPECIMENS},
...
export function nextFixedLabSpecimen(current:LabSpecimen):LabSpecimen|null{
  if(current.hole.premix){const item=premixItem(current.hole),bank=item?.collection==='legs'?LEG_SPECIMENS:item?.role?REGION_SPECIMENS:PREMIX_SPECIMENS;const i=bank.findIndex(s=>s.hole.courseId===current.hole.courseId);return bank[(i+1)%bank.length]??null;}
