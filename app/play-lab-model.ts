import {PREMIX_ENGINE,PREMIX_SPECIMENS,REGION_SPECIMENS,generatePremixHole,allPremixRows,premixItem,premixProgress} from './play-premix-bank';
...
export function nextFixedLabSpecimen(current:LabSpecimen):LabSpecimen|null{
  if(current.hole.premix){const bank=premixItem(current.hole)?.role?REGION_SPECIMENS:PREMIX_SPECIMENS;const i=bank.findIndex(s=>s.hole.courseId===current.hole.courseId);return bank[(i+1)%bank.length]??null;}
