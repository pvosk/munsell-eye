...
import {hardLabBank,generateHardHole,inverseLabBank,generateInverseHole,contrastLabBank,generateContrastHole,journeyLabBank,generateJourneyHole,protectedLabBank,generateProtectedHole} from './play-engine';

import {campaignSourceId,campaignForHole,campaignSnapshot,nextCampaignSpecimen,campaignAnalysis} from './play-campaign';
export const LAB_HARD_ENGINE='glider-lab-11-controls-1';
...
export const supportedLabEngine=(engine:string)=>[LAB_HARD_ENGINE,LAB_INVERSE_ENGINE,LAB_CAMPAIGN_ENGINE,LAB_ENGINE,'glider-courses-2-controls-1',LAB_ROUND_ENGINE,LAB_PAIRED_ENGINE,LAB_DESIGN_ENGINE,LAB_FOCUSED_ENGINE,LAB_DIRECTED_ENGINE,LAB_PROTECTED_ENGINE,LAB_JOURNEY_ENGINE,LAB_CONTRAST_ENGINE].includes(originalEngine(engine));
...
export const directedForHole=(raw:string)=>{const id=campaignSourceId(raw);return hardLabBank.holes.find(h=>h.record.id===id)??inverseLabBank.holes.find(h=>h.record.id===id)??contrastLabBank.holes.find(h=>h.record.id===id)??journeyLabBank.holes.find(h=>h.record.id===id)??protectedLabBank.holes.find(h=>h.record.id===id)??directedLabBank.holes.find(h=>h.record.id===id);};
...
export function nextFixedLabSpecimen(current:LabSpecimen):LabSpecimen|null{
  ...
  const bank=current.hole.courseId.startsWith('lab-11-')?LAB_HARD:current.hole.courseId.startsWith('lab-10-')?LAB_INVERSE:current.hole.courseId.startsWith('lab-9-')?LAB_CONTRAST:current.hole.courseId.startsWith('lab-8-')?LAB_JOURNEYS:current.hole.courseId.startsWith('lab-7-')?LAB_PROTECTED:current.hole.courseId.startsWith('lab-6-')?LAB_DIRECTED:current.hole.courseId.startsWith('lab-5-')?LAB_FOCUSED:current.hole.courseId.startsWith('lab-4-')?LAB_DESIGN:current.hole.courseId.startsWith('lab-3-')?LAB_PAIRED:current.hole.courseId.startsWith('lab-2-')?LAB_STARTERS:null;
  if(!bank)return null;
