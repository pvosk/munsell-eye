...
import workbenchBank from './generated/play-course-workbench.json';
...
PLAY_LEVELS.push(...workbenchBank.palettes as PlayLevel[]);
...
export function generateContrastHole(levelIndex:number,stage:number,seed=20261221):Hole {
  ...
  if(!item)throw new Error('Unknown contrast lab hole');
  const r=item.record,target=mixtureColor(PLAY_LEVELS[levelIndex].paints,r.target);
  return {seed,stage,start:neutralStart(PLAY_LEVELS[levelIndex]),target,notation:nearestNotation(target),par:r.par,recipe:[...r.recipe],tolerance:item.analysis.tolerance,timingWindow:r.timingWindow,courseId:r.id,kind:r.kind,solutionShots:r.solutionShots,routeOrder:[...r.order],routeTimes:[...r.times]};
}
