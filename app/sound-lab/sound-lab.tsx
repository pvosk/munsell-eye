...
import { SoloPanel } from "./solo-panel";
...
type Tab = "journey" | "sound" | "mapping" | "solo";
...
export default function SoundLab({ signIn }: { signIn: ReactNode }) {
  const [setup, setSetup] = useState<Setup>(() => sanitizeSetup(DEFAULT_SETUP)),
    ...
    [soloIncoming, setSoloIncoming] = useState<Setup | undefined>(),
  ...
  return (
    <main className="sl-root sl-v2">
      <header className="sl-header">
        {/* Full navigation initializes the existing game runtime. */}
      ...
      <div className="sl-workspace">
        <div
          className="sl-master"
          style={tab === "solo" ? { display: "none" } : undefined}
        >
        ...
        <div className="sl-transport">
          ...
          >
            {(["journey", "sound", "mapping", "solo"] as Tab[]).map((t) => (
              <button
                key={t}
                id={`tab-${t}`}
                role="tab"
          ...
          <div
            className="sl-transport-actions"
            style={tab === "solo" ? { display: "none" } : undefined}
          >
            <button
              className="sl-primary"
        ...
        {tab === "solo" ? (
          <SoloPanel
            incoming={soloIncoming}
            onConsumed={() => setSoloIncoming(undefined)}
            signIn={signIn}
            onUseSound={(solo) => {
              const current = setupRef.current;
              const next = applyPreset(current, {
                id: "solo-transfer",
                name: "Solo sound",
                notes: "",
                scope: "sound",
              ...
              // Pop pitch-envelope controls are part of its source, not a vocal journey glide.
              if (solo.parameters.instrument === "pop") {
                next.parameters.glideStart = solo.parameters.glideStart;
                next.parameters.glideTime = solo.parameters.glideTime;
              }
              delete next.solo;
              update(next);
              setTab("sound");
              setNotice(
                "Solo sound copied. Your Journey music and mappings are retained; active mappings can override the copied values.",
              );
            }}
          />
        ) : (
                ...
                <div className="sl-shot-controls">
                  ...
                  <div className="sl-button-row">
                    <button
                      className="sl-primary"
                      disabled={
                        !enabled || snapshot?.audioPaused || !!paintError
                      }
                      onClick={() => play()}
                    >
                      Play shot
                    ...
                    >
                      Hold & release
                      {holding ? ` · ${heldSeconds.toFixed(1)}s` : ""}
                    </button>
                  </div>
                  <div className="sl-button-row">
                    <button
                    ...
                    <label className="sl-toggle">
                      <input
                        ...
                        checked={j.repeat}
                        onChange={(e) =>
                          setJourney({ repeat: e.target.checked })
                        }
                      />
                      Repeat shot
                    </label>
                  ...
                  <p>
                    Pause holds the journey position; existing tails continue.
                    Stop ends its sources. Reset audio clears all buffers and
                    keeps settings.
                  </p>
              ...
              {tab === "journey" && (
                <>
                  ...
                  <MusicPanel
                    ...
                    bodyOnly={
                      p.instrument === "convergence" && p.shotArps === 0
                    }
                  >
                    <HarmonyJourney
                      music={p.music}
                  ...
                  <MelodyGrid
                    ...
                    customScale={p.intervals.map(
                      (cents) =>
                        69 + 12 * Math.log2(p.root / 440) + cents / 100,
                    )}
                    timing={j.rhythm}
                  ...
                  <details className="sl-card">
                    <summary>
                      Custom tuning · foundation and six intervals
                    </summary>
                    <label className="sl-toggle">
                      <input
                        type="checkbox"
                  ...
                  <section className="sl-card">
                    ...
                    <div className="sl-button-row">
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (engine.current)
                            engine.current.ambientPaused = false;
                        }}
                      >
                        Start ambient
                      </button>
                    ...
                    />
                    <p>
                      Independent of the shot and motif. Pausing stops new
                      notes; stopping also ends its sounding sources. Shared
                      effects can still ring.
                    </p>
                  </section>
                </>
              )}
              {tab === "sound" && (
                <>
                  ...
                  <div className="sl-notice">
                    Source attack, decay and timbre affect new notes. Use Play
                    shot again to compare an onset, or use Solo sounds for
                    automatic retriggering.
                    {mappingMode !== "off" &&
                      setup.mappings.some((m) => m.enabled) && (
                        <>
                          <p>
                            Movement mappings can override these sliders. The
                            mapped value is shown beneath each affected control.
                          </p>
                          <button
                            onClick={() => {
                              setMappingMode("off");
                          ...
                          >
                            Bypass mappings for sound edits
                          </button>
                        </>
                      )}
                  </div>
                  ...
                  <div className="sl-grid-3 sl-card">
                    <label>
                      ...
                      >
                        <option value="chord">
                          Follow current chord voices
                        </option>
                        <option value="manual">
                          Independent manual anchor
                        </option>
                      </select>
                    </label>
                    <label>
                  ...
                  <div className="sl-control-groups">
                    {[...GROUPS]
                      ...
                      .filter(
                        (g) =>
                          g.name !== "Vocal glide" || p.instrument === "vocal",
                      )
                      .map((group) => (
                        <details
                          key={group.name}
                            ...
                            {group.sliders
                              ...
                              .map((spec) => (
                                <Slider
                                  ...
                                  value={p[spec.key]}
                                  mapped={
                                    snapshot?.mapped[spec.key as Destination]
                                  }
                                  onChange={(v) => change(spec.key, v)}
                                />
                              ))}
              ...
              {tab === "mapping" && (
                <>
                  <section className="sl-card">
                    <h2>Route studies</h2>
                    <p>
                      Load a complete sound, sketch and mapping inspired by the
                      game debrief. These are audition proposals; they do not
                      change game math.
                    </p>
                    <div className="sl-models">
                  ...
                  <div className="sl-section-head">
                    <div>
                      ...
                      <p>
                        One connection per destination. Base sliders remain
                        intact; the live value appears alongside them.
                      </p>
                    </div>
                    <div className="sl-button-row">
                      <button
                  ...
                  <section className="sl-card">
                    <label>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setJourney({
                            path: e.target.value as Journey["path"],
                          })
                        }
                      >
                        <option value="probe">Editable color probe</option>
                        <option value="paint">
                          Existing game paint mixing
                        </option>
                      </select>
                    </label>
                    {j.path === "probe" ? (
                      ...
                      <>
                        ...
                        <div className="sl-grid-2">
                          <label>
                            ...
                            >
                              {palettes[j.paint.level]?.paints.map(
                                (paint, i) => (
                    ...
                    <p>
                      Boundary reference: sRGB at the current OKLab lightness
                      and hue. This is distinct from the palette’s reachable
                      hull and the game’s displayed Munsell field.
                    </p>
                  </section>
                  ...
                  <p>
                    ...
                    near the boundary. Neutral ascent accumulates upward travel
                    at low absolute chroma. Hue fades near neutral; signed
                    developments use 0.5 as no change.
                  </p>
