...
## Shot-first usability and source separation

User requested persistent keyboard/shot/loop transport, importance-ordered combined music/resolution UI, progression tiles, draggable start/landing/target, live loop edits, clearer source identities and debrief-based mapping studies. Hue/value with separate chroma controls was explicitly chosen. Implemented those refinements, idle Resolve demo, standalone 18-voice wandering convergence with optional note layer, stronger glass excitation, six route studies, and target-proximity audition mapping. Refer to the sound README for behavior and validation. Base is 2f62ed3, integrated publicly by main as c70f6ce / v69. No gameplay, API, schema, hosting or dependency changes; new target and sound controls use existing preset serialization. Updated chime/flight SynthDefs and manifest must ship with engine changes. Main owns coordinated publication.


## Thirty studies and modular preset workshop

User requested roughly 30 complete musical/route presets, then clarified a tonal-leaning mix with modular sound, music and mapping ingredients plus a few featured complete setups. Added ten families × three original studies, six featured entry points, grouped sticky selection, an inspect-before-load family browser, listening/tuning/trigger notes, scoped loading and portable family/all-30 downloads. Added saved `harmony` scope (music + musical shot clock) without a database migration; details and family catalogue are in the sound README. Existing scopes and old presets remain supported. Built-ins do not overwrite or auto-populate account presets. Full and partial study loads retain listening volume and Loop.

Base is sound 16c9ada, already integrated by main; main/public baseline is v71 / 49eae2e86c35d597ce317d6877d09721505aac7d. Scope is sound-lab UI/data, preset validator/library and sound tests/docs. No game, engine, SynthDef, dependency, hosting or schema changes. Known compressed final convergence gathering remains documented and separate from game capture rules. Main remains the sole integration/publication owner.

Validation passed: production build, TypeScript, sound lint, 1,728 harmonic combinations, existing journey suite, 30-study/93-mapping/90-partial-recall tests, local D1 preset tests with the new harmony scope, and all 30 full-duration browser SC shots. DSP checks observed 25 capture states, five misses, zero foreground notes in the three body-only convergence studies, finite/nonzero signals, maximum sampled peak 0.265, and silence after Stop. No page/runtime errors. No subjective listening, visual/touch QA or hosted account-sync claim. Runtime summary is local at /tmp/chroma-studies-audio.json.


## Independent Solo sounds workspace

User requested isolated glass ribbon, chromatic convergence, release pop, piano tunnel and harp-glissando refinement, plus predictable live edits. Synced the clean sound branch with main 90adc89 (v82) as a340907; main's published header/mode-nav/CSS are preserved. Main subsequently reported a36f753/v83 with no sound changes. Added a Solo sounds tab, separate player/drafts, fixed-reference source auditions, source auto-retrigger versus live FX, per-audition pause/stop/reset, portable/account-preserved optional solo metadata, and explicit copy-back into Journey. New plucked harp and pitched-noise pop definitions accompany engine loading changes. Existing synth binaries and game rules remain unchanged. Solo convergence bypasses the known capture handoff compression using a continuous curve and held finish; Journey capture easing is not altered.

See the sound README for exact controls, persistence, scope boundaries and validation. Main remains the publisher. Integration should cherry-pick the solo feature commit only, not the synchronization merge; main already owns all prerequisite game/navigation changes. No schema migration, dependency change, hosting edit or game-source change is part of the solo feature.

Final verification passed: build/tsc/sound lint; old music/Journey/study suites; solo patterns and glissando direction; local D1 solo-metadata recall; unchanged Journey runtime regression; and final five-model solo runtime checks after loop-gap clarification. No runtime errors, live FX did not restart the source, source edits retriggered, Pause froze the clock, Loop repeated, Stop yielded zero samples and stopped edits did not restart audio. Artifacts remain local in /tmp/chroma-solo-final-audio.json and /tmp/chroma-solo-regression.json.

Small Solo UI follow-up: single-pitch pop hides melodic mode/chord selectors; harp identifies its chord as resonator-only; sweep speed is disabled with an explanatory hint when Animate bands is off. This directly addresses controls that otherwise appear to have no effect. No synthesis or scheduling changes; TypeScript, scoped lint and production build verified.
