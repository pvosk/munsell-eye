...
The [ten-hole pigment-leg lab](play-pigment-leg-lab.md) now packages a varied subset for play: six palette-first specimens, three route-first specimens and a familiar Zorn reference. It includes explicit shorter alternatives to the two four-leg examples, preserves historical collections, and adds leg-versus-release evidence without changing controls, pigment strengths or scoring tolerance. Broader branching setup-region planning is discussed, not yet implemented.
