...
## Solo sounds — 13 September

Solo sounds is a fourth tab in the existing sound lab. It has a separate player and five independent draft setups: Glass ribbon, Convergence, Release pop, Piano tunnel and Harp flourish. Entering it stops the Journey engine; leaving disposes the solo player and its scheduled events. The app-mode header/dropdown from main remains intact. The original Journey setup stays in place.

Every solo audition uses one fixed key/mode/chord (or imported custom tuning), no movement mappings, no progression clock and no game capture event. Glass has switchable glass/piano/synth/harp inputs, ringing-band/reverse-swell/synthetic-ribbon experiments, separate direct/ribbon levels, bandwidth, manual or chord resonator anchor, animated sweep rate/direction/phase/depth, granular routing/reversal and space controls. These are original perceptual experiments, not a verified Glass Eyes production chain.

Convergence uses the existing 18-voice cloud with one normalized gathering curve that reaches its destination at the end of Gathering seconds. It then holds for the chosen time and fades over half a second. Gathering onset/curve, spread, wandering, voice count and body are adjustable; no extra motif is emitted. This solo audition avoids the known Journey flight/capture handoff compression without changing the game's capture rules or the existing Journey arrival behavior.

Piano tunnel uses a fixed procedural pattern, count, octave span and spacing curve. Harp uses a distinct generated plucked source and ascending/descending/up-then-down scale sweeps. Its synthesis uses SC's core [Pluck](https://doc.sccode.org/Classes/Pluck.html) delay-string model, not recorded harp samples. Release pop uses one sine-body pulse, a filtered noise transient and a controllable exponential pitch fall ([XLine](https://doc.sccode.org/Classes/XLine.html)); its air, pitch offset, fall time and decay are independent. Harp and pop are also reusable source choices on Sound; Copy this sound to Journey retains Journey harmony/mappings (and copies pop pitch-envelope values explicitly).

### Predictable controls and transport

Effects update the active processing chain live. Source envelope/timbre and reference-pattern changes affect new onsets. The optional Retrigger after source edits control waits 350ms after source/gesture edits, then restarts a clean audition; it never starts audio before the first Play or after an explicit Stop. Convergence body controls update live instead of retriggering. The user can disable retriggering and hear changes on later notes or loops. No claim is made that an existing struck envelope is retroactively reshaped.

Play/P/Space starts or retriggers. Pause suspends the audio context and scheduler clock, preserving effect memory. Stop + tails/Escape clears sources and effect memory while retaining settings. Reset audio destroys/recreates the engine, retaining all five drafts. Restore this sound's controls resets only the selected draft. Loop reads current settings; Clear old tails each loop controls accumulation. The loop gap follows the last scheduled note (or pop ring time, or convergence hold/fade). Hear source only bypasses processing with an explicit notice. Clear effect memory and solo recording are available separately.

The main Sound panel now explains mapped overrides and new-onset controls, with a direct Bypass mappings for sound edits button. Existing mapped-value hints remain. Solo has no mappings to override its sliders.

Five drafts persist locally under `chroma-solo-drafts-v1`. Complete library presets carry optional sanitized `Setup.solo` metadata alongside parameters, using the existing account API and version-2 JSON. No database migration is needed. Loading a complete solo preset from Journey opens the Solo tab; consumption is cleared so returning later does not overwrite new edits. Use Complete setup to retain the full solo gesture. Partial sound/music recalls retain their existing scope semantics. Older clients need a refresh to retain the new optional metadata and harp/pop source IDs.

The engine loads two additional SynthDefs, `chroma_harp` and `chroma_pop`; existing SynthDef binaries are unchanged. Ship the new files and updated manifest with the JS changes. The small SC graph writer remains the source of truth. No game source or pigment/mixing/scoring behavior was edited by this checkpoint.

Validation: TypeScript, sound lint and production build; existing 1,728 harmony combinations, Journey and 30-study/93-mapping suites; solo fixed-pattern/glissando/portable-preset tests; local D1 save/recall including solo metadata; full existing runtime regression; five solo audio auditions plus live FX, source retrigger, pause, loop, stop-silence and no-restart-after-Stop checks. Initial solo sampled peaks stayed below 0.285; every model produced finite nonzero audio, convergence emitted no note events, pop emitted one, and Stop measured digital silence. No browser visual, physical-touch, subjective-listening, reference-match or hosted account-sync claim.
